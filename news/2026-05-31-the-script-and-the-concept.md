...
**The error is Chemism.** The mechanical script meets its real other — the live system it acts on — and shows that it is *not* self-sufficient, *not* indifferent to its conditions. It reacts; a contradiction surfaces as a broken run. That non-indifference, the object revealed as standing in essential relation to what it acts on, is the content of [**Chemism**](../synopsis/09-doctrine-of-the-concept.md#chemism). (Strictly, Hegel's chemism is the *affinity* of non-indifferent objects tending toward a neutral product, not breakdown as such; here the execution error is the symptom that *discloses* the script's chemical non-self-sufficiency.) The execution error is not a nuisance external to the logic; it is the moment the released object discloses that it was never really self-standing.
...
One successful instance is a **singular**. To leap from it to a universal rule is to reach for a universal by mere *enumeration* — and Hegel's verdict on the inductive universal (in the [discussion of the syllogism](../synopsis/09-doctrine-of-the-concept.md#the-syllogism-the-concept-rejoined-to-itself), where induction is the reflective form whose middle term is "all the cases") is that even a completed induction yields only an *allness* — a collected "in all the cases we looked at" — never a genuine concept that necessitates itself. The honest move is to file the once-seen pattern as a **candidate** — grounded, real, but not yet universal — and to withhold the crown until the shape shows itself to be *self*-necessitating rather than merely repeated, ideally by recurring somewhere quite different and there disclosing its own ground. This is exactly the distinction the synopsis draws between [the spurious and the true infinite](../synopsis/13-finitude-and-the-true-infinite.md): piling up confirming cases — "and another, and another" — is the **spurious** infinite, the endless line that never closes into anything; the **true** universal is *not larger but whole* — reached when the pattern exhibits its own ground and necessitates itself, not when the heap of instances grows taller. The candidate-queue discipline is the refusal to mistake a growing pile of cases for a closed concept.
